...
test('bomb awarded every third clearing placement, never per line',()=>{let g=fresh();for(let i=1;i<=6;i++){g.board=Array.from({length:8},()=>Array(8).fill(0));g.board[0]=[0,1,1,1,1,1,1,1];g.board[7][7]=2;g.pieces=[{shape:[[1]],color:1},null,null];g=place(g,0,0,0)!.game;assert.equal(g.bombs,i<3?0:1);}assert.equal(g.combo,6);});
...
test('spent bomb can be earned again at the next combo milestone',()=>{let g=fresh();g.bombs=1;g.combo=3;g.board[7][7]=2;g=detonate(g,4,4)!.game;assert.equal(g.bombs,0);for(let i=4;i<=6;i++){g.board[0]=[0,1,1,1,1,1,1,1];g.board[7][7]=2;g.pieces=[{shape:[[1]],color:1},null,null];g=place(g,0,0,0)!.game;assert.equal(g.bombs,i===6?1:0);}});
