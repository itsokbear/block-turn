...
test('four lines and new bomb are combined without losing reward',()=>{const g=fresh();const e=moveEvent(g,{...g,lines:4,bombs:1},false,true)!;assert.equal(e.kind,'lines');assert.equal(e.bomb,true);assert.match(e.details,/4 линий/);assert.match(e.details,/Бомба готова/);});
