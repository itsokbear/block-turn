...
function worker(base='/',fail=false){
 const events={},stores=new Map(),prefix='block-turn-offline-'+encodeURIComponent(base)+'-',name=prefix+'new';let claimed=false,skipped=false,online=true;
...
test('failed install removes partial cache and preserves previous version',async()=>{const w=worker('/block-turn/',true);w.stores.set(w.prefix+'old',new Map());await assert.rejects(w.life('install'));assert.equal(w.stores.has(w.name),false);assert.equal(w.stores.has(w.prefix+'old'),true);});
test('update activates only on request and removes only this app scope caches',async()=>{const w=worker('/block-turn/');w.stores.set(w.prefix+'old',new Map());w.stores.set('block-turn-offline-%2Fother%2F-old',new Map());await w.life('install');assert.equal(w.skipped,false);w.message({type:'ACTIVATE_UPDATE'});assert.equal(w.skipped,true);await w.life('activate');assert.equal(w.stores.has(w.prefix+'old'),false);assert.equal(w.stores.has('block-turn-offline-%2Fother%2F-old'),true);});
